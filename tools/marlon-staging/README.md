# Dashboard de Controle de Tráfego Pago — MARLON MATTEDI / Funil de Venda Direta

> Repositório **já configurado** para o cliente MARLON MATTEDI (Funil de Venda
> Direta / tráfego direto). A conta roda duas siglas de campanha, `C20` e `EXP`,
> e a dashboard cobre as duas — ver `CLAUDE.md`.
> Os valores do cliente ficam em `build/config.py`; a engine
> (`build/build.py`, `build/template.html`, `ia-worker/worker.js`) continua
> genérica. Publicado em https://scale-ag.github.io/dash-marlon-mattedi/
...
Produto principal: **Treino Controle 2.0** (`TREINO CONTROLE 2.0` na planilha).
...
4. Depois do primeiro deploy bem-sucedido, a URL pública aparece em **Settings
   → Pages** e é sempre `https://scale-ag.github.io/dash-marlon-mattedi/`.
5. Configure o cron-job.org para dar continuidade aos builds automáticos a cada
   ...
   placeholders `scale-ag`/`dash-marlon-mattedi` para você substituir
...
3. Use esses **mesmos valores** para substituir manualmente os placeholders
   `scale-ag` e `dash-marlon-mattedi` que aparecem em `SETUP-CRON.md`,
